...
airdrop_data = [
    # address: amount
    ("0xF24c9B7A6E27fca79cc310B25760DDeF32a28603", 100),
    ("0x01cA1B6371Ae950aFDb02D93fd7322c782da6807", 200),
    ("0xA4183cC8F12F86895DbfCFA3e19fbAeE019fb2DA", 300),
    ("0xA57623F5021faAD634c8A3952A7262c355dE00b6", 400),
]

leafs = []
for i in airdrop_data:
    abiEncoded = eth_abi.encode_abi(["address", "uint256"],  [i[0], i[1]])
    leafs.append(Web3.solidityKeccak(['bytes'], ['0x' + abiEncoded.hex()]))
    
# Initialize MerkleTools and build the Merkle tree
merkle_tools = MerkleTools()
merkle_tools.add_leaf(leafs)
...
merkle_data = {"root": merkle_root, "proofs": merkle_tools.get_proof(0)}
with open("merkle_tree.json", "w") as json_file:
    json.dump(merkle_data, json_file)

print(f"Merkle root: {merkle_root.encode('utf-8')}", f"Merkle proof: {merkle_tools.get_proof(0)}")
print(f"Merkle root: {merkle_root.encode('utf-8')}", f"Merkle proof: {merkle_tools.get_proof(1)}")